...
projection, soft Markov, **entropy-onset t0 + type-deviation `dist` fork** for View C). This is the
**only** pipeline — the pre-expert `clean.py` / `pipeline_json.py` baselines were removed. See config.md.
...
`alignment.covariatesAtT0` still keeps the T=0 `directorHeterogeneity`. Adjust in
